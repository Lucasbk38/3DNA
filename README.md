...
- le dossier **gen** contient:
    ...
    - le fichier `fitness.py` définit la manière d'évaluer un individu (l'opposé de la distance euclidenne entre le point départ et le point d'arrivée de la séquence d'ADN après avoir effectué les opérations de la table de rotation). On cherche à maximiser la fitness. Il y a deux sortes d'évaluation dans le fichier. La première regarde seulement le dernier point. La deuxième regarde les deux derniers points pour prendre en compte la rotation (et est donc moins facile à optimiser mais plus fiable physiquement)
