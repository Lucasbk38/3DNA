...
class Mutation(ABC):
    ...
    def mutate(self, individu: RotTable) -> RotTable:
        return RotTable({ k: [e if e == 2 else np.clip(self.mutateValue(e), rotTableConfig[k][i] - rotTableConfig[k][3 + i], rotTableConfig[k][i] + rotTableConfig[k][3 + i]) for i, e in enumerate(individu.rot_table[k])] for k in individu.rot_table })
